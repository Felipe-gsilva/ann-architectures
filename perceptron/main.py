    ...
    try:
        for i in range(1, 4):
            ...
            if i != 3:
                ...
                print(f"\n=== Training Dataset {i} ===")
                ep, tr_acc = model.train(df, learning_rate=hyperparams["LR"], epochs=hyperparams["epochs"], seed=hyperparams["seed"])
                print("--- Testing ---")
                te_acc = model.test(df_test)
                
                metrics_log.append({"Dataset": i, "LR": hyperparams["LR"], "Max_Epochs": hyperparams["epochs"], 
                                    "Epochs_Run": ep, "Train_Acc": tr_acc, "Test_Acc": te_acc})
            ...
            if i == 3: 
                ...
                for (lr, max_ep) in product(learning_rates, epochs): 
                    ...
                    ep, tr_acc = model.train(df, learning_rate=lr, epochs=max_ep, seed=hyperparams["seed"])
                    print("--- Testing ---")
                    te_acc = model.test(df_test)
                    
                    metrics_log.append({"Dataset": i, "LR": lr, "Max_Epochs": max_ep, 
                                        "Epochs_Run": ep, "Train_Acc": tr_acc, "Test_Acc": te_acc})
